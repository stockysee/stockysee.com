...
// Find the BuilderSidebar variable declaration
const bsDecl = sourceFile.getVariableDeclaration("BuilderSidebar");
const bsInit = bsDecl.getInitializer();

const animatePresence = bsInit.getDescendantsOfKind(SyntaxKind.JsxElement).find(n => n.getOpeningElement().getTagNameNode().getText() === "AnimatePresence");
...
while (currentExpr && currentExpr.getKind() === SyntaxKind.ConditionalExpression) {
  ...
  if (condText.includes("activePanel === 'editor'")) {
...
export interface \Props extends BuilderSidebarProps {
...
console.log("Restored EditorPanel.tsx from backup!");

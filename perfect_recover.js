...
const startIndex = text.indexOf("activePanel === 'editor' && editingSection ? (");
if (startIndex === -1) {
...
// Find the '('
const firstParen = text.indexOf('(', startIndex);
let openBraces = 0;
let started = false;
let endIndex = -1;
...
if (endIndex === -1) {
    console.error("Could not find matching parenthesis!");
    process.exit(1);
}

// The content of the Editor panel is inside the parenthesis.
// So we substring from firstParen + 1 to endIndex.
const editorJsx = text.substring(firstParen + 1, endIndex);
...
fs.writeFileSync('app/dashboard/storefront/builder/panels/EditorPanel.tsx', componentTemplate("EditorPanel", editorJsx));
console.log("Restored EditorPanel.tsx! Length:", editorJsx.split('\n').length);
